...
- (void)setupCustomView
{
    ...
    _circleImageView = [[UIImageView alloc] initWithImage:image];
    self.customView = [[UIView alloc] initWithFrame:_circleImageView.bounds];
    [self.customView addSubview:_circleImageView];
}
...
- (void)startCircleTimer
{
    if (self.customView != nil) {
        circleAngle = 0;
        _circleImageView.transform = CGAffineTransformIdentity;
        circleTimer = [NSTimer scheduledTimerWithTimeInterval:0.01
                                                       target:self
                                                     selector:@selector(handleCircle:)
                                                     userInfo:nil
                                                      repeats:YES];
        [circleTimer retain];
    }
...
- (void)handleCircle:(id)sender
{
    ...
    _circleImageView.transform = CGAffineTransformMakeRotation(circleAngle / (2*M_PI));
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.mode = FxActivityModeCustomView;
        self.labelText = nil;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat fwidth = self.width>self.height ? self.width:self.height;
    // Draw rounded HUD bacgroud rect
    CGRect boxRect = CGRectMake(roundf((allRect.size.width - fwidth) / 2) + self.xOffset,
                                roundf((allRect.size.height - fwidth) / 2) + self.yOffset, fwidth, fwidth);
    // Corner radius
    float radius = 10.0f;
    
    CGContextBeginPath(context);
    CGContextSetGrayFillColor(context, 1.0f, self.opacity);
